...
/*		//長調なら
		if(AnalyzeTPS.ChordOutput[s][3].equals("M")){
			...
			//hmmmmmmmm
			//hmmm
			//this is why you shouldn't let me think with commentable code at hand
			//with the switch cost, if there is more than 1 key it will def cost more to switch one to minor
			//so either all major or all minor, or sth like that
			//BUT WHAT IF M - m costs the same as m-m -- oh it can't
			//okay BUT what if M-m-M-m costs the same as m-m-m-m b/c one of the m's costs 1 more which makes up for the two M's
			//what then huh
			//what then
			//...
			//idk
			//this is too twisted for me
			//no equality test for now then huh
			toRelative(s,g);
			innerCCG(s,g,Lm);
			toRelative(s,g);
		}
	...
	private static String[][] degreeNameConversion(int s, int g){
		...
		for(int i=0;i<Degree.length;i++){
			...
			if(Integer.parseInt(Degree[i][0]) >= Integer.parseInt(Degree[i][2])){
				...
				Degree[i][0] = 12+(Integer.parseInt(Degree[i][0])-Integer.parseInt(Degree[i][2]))+"";
			}
		}
